...
// 🌏 Asie de l'Est – Extraction & zones TINSFLASH PRO+++
...
// ===========================
// 🧠 Extraction Asie de l'Est
// ===========================
export async function runGlobalAsiaEst() {
  try {
    await addEngineLog("🌏 Démarrage extraction Asie de l'Est", "info", "AsiaEst");

    // Ici on simule le traitement des zones — en pratique tu appelles GFS/ECMWF/ICON
    const allPoints = [];
    ...
    await saveEngineState({ lastRunAsiaEst: new Date(), checkup: { AsiaEst: "ok" } });
    await addEngineLog("✅ Extraction Asie de l'Est terminée", "success", "AsiaEst");
    return { success: true, zones: allPoints };
  } catch (err) {
    await addEngineError("💥 Erreur extraction Asie de l'Est : " + err.message, "AsiaEst");
