...
app.post("/api/run-global", async (req, res) => {
  try {
    console.log("🚀 Lancement RUN GLOBAL via API");
...
// === PAGES ADMIN ===
app.get("/admin", (_, res) => res.sendFile(path.join(__dirname, "public", "admin-pp.html")));
app.get("/admin-alerts", (_, res) =>
  res.sendFile(path.join(__dirname, "public", "admin-alerts.html"))
);

const PORT = process.env.PORT || 5000;
app.listen(PORT, () => console.log(`🚀 TINSFLASH Server en ligne sur port ${PORT}`));
